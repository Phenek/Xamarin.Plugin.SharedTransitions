...
using Plugin.SharedTransitions.Droid.Renderers;
using Xamarin.Forms;
...
namespace Plugin.SharedTransitions.Droid.Renderers
{
    public class SharedTransitionNavigationPageRenderer : NavigationPageRenderer
    {
        ...
        protected override void SetupPageTransition(FragmentTransaction transaction, bool isPush)
        {
            ...
            else
            {
                ...
                foreach (var tagMap in mapStack)
                {
                    ...
                    if (fromView != null)
                    {
                        var correspondingTag = Plugin.SharedTransitions.Transition.GetUniqueTag(tagMap.Tag, _selectedGroup, isPush);
        ...
        public override void AddView(View child)
        {
            if (!(child is Toolbar) && Build.VERSION.SdkInt >= BuildVersionCodes.Lollipop)
            {
                ...
                var transitionInflater = TransitionInflater.From(Context);

                var ressourceTransition = transitionInflater.InflateTransition(Resource.Transition.navigation_transition);

                var navigationTransition = ressourceTransition.SetDuration(_sharedTransitionDuration);
